...
const AddStudent = ({setAddStudent, studentSection}) => {
    ...
    const [studsection, setStudSection] = useState(studentSection)

    console.log(studsection)

    const handleSubmit = (event) => {
        event.preventDefault();

        fetch('http://localhost:4001/api/addstudent', {
            method: 'POST',
            headers: {
              ...
              <div className="mb-2 relative">
                <input
                 value={studsection}
                  ...
                  onChange={event => setStudSection(event.target.value)}
                  className="block w-full px-4 py-2 mt-2  bg-white border rounded-md focus:border-[#6a7ff4] focus:ring-[#6a7ff4] focus:outline-none focus:ring focus:ring-opacity-40"
               />
